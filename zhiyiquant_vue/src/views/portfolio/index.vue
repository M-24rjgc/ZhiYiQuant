<template>
  <div class="portfolio-container" :class="{ 'theme-dark': isDarkTheme }">
      ...
      <div class="positions-section">
        <div class="section-header">
          ...
          <div class="header-actions">
            ...
            <a-button style="margin-left: 8px;" @click="openClosedPositionsModal">
              <a-icon type="history" />
              已平仓记录
            </a-button>
          </div>
              ...
              >
                <div class="position-header">
                  ...
                  <div class="position-actions">
                    ...
                    <a-popconfirm
                      title="按当前价平仓并保存到历史记录？"
                      @confirm="closeManualPosition(pos)"
                    >
                      <a-button type="link" size="small" :loading="closingPositionId === pos.id">
                    ...
                    >
                      <div class="position-header">
                        ...
                        <div class="position-actions">
                          ...
                          <a-popconfirm title="按当前价平仓并保存到历史记录？" @confirm="closeManualPosition(pos)">
                            <a-button type="link" size="small" :loading="closingPositionId === pos.id"><a-icon type="check-circle" /></a-button>
                          </a-popconfirm>
    ...
    <a-modal
      title="已平仓记录（已实现盈亏）"
      :visible="showClosedPositionsModal"
      @cancel="showClosedPositionsModal = false"
      :footer="null"
      width="900px"
    >
      <a-spin :spinning="loadingClosedPositions">
        <div v-if="closedPositions.length === 0" class="empty-state small">
          <a-empty description="暂无已平仓记录" :image="simpleImage" />
        </div>
        <a-table
          v-else
          :data-source="closedPositions"
          :pagination="false"
          size="small"
          row-key="id"
        >
          <a-table-column title="标的" key="symbol">
            <template slot-scope="text, record">
              <a-tag :color="getMarketColor(record.market)" size="small">{{ getMarketName(record.market) }}</a-tag>
              <span>{{ record.symbol }}</span>
            </template>
          </a-table-column>
          <a-table-column title="方向" key="side">
            <template slot-scope="text, record">
              <a-tag :color="record.side === 'long' ? 'green' : 'red'" size="small">
                {{ record.side === 'long' ? $t('portfolio.positions.long') : $t('portfolio.positions.short') }}
              </a-tag>
            </template>
          </a-table-column>
          <a-table-column title="数量" data-index="quantity" key="quantity">
            <template slot-scope="text">{{ formatNumber(text, 4) }}</template>
          </a-table-column>
          <a-table-column title="买入价" data-index="entry_price" key="entry_price">
            <template slot-scope="text, record">{{ getCurrencySymbol(record.market) }}{{ formatPrice(text) }}</template>
          </a-table-column>
          <a-table-column title="卖出价" data-index="close_price" key="close_price">
            <template slot-scope="text, record">{{ getCurrencySymbol(record.market) }}{{ formatPrice(text) }}</template>
          </a-table-column>
          <a-table-column title="已实现盈亏" key="realized_pnl">
            <template slot-scope="text, record">
              <span :class="record.realized_pnl >= 0 ? 'positive' : 'negative'">
                {{ record.realized_pnl >= 0 ? '+' : '' }}{{ getCurrencySymbol(record.market) }}{{ formatNumber(record.realized_pnl) }}
                ({{ record.realized_pnl_percent >= 0 ? '+' : '' }}{{ Number(record.realized_pnl_percent || 0).toFixed(2) }}%)
              </span>
            </template>
          </a-table-column>
          <a-table-column title="平仓时间" key="close_time">
            <template slot-scope="text, record">{{ formatTime(record.close_time || record.closed_at) }}</template>
          </a-table-column>
        </a-table>
      </a-spin>
    </a-modal>
...
import {
  ...
  closePosition as closePositionApi, getClosedPositions,
  getPortfolioSummary,
...
export default {
  ...
  data () {
    return {
      ...
      positionForm: null,
      showClosedPositionsModal: false,
      loadingClosedPositions: false,
      closedPositions: [],
  ...
  methods: {
    ...
    async closeManualPosition (pos) {
      if (!pos || !pos.id) return
      this.closingPositionId = pos.id
      try {
        const payload = {
          close_price: Number(pos.current_price) > 0 ? Number(pos.current_price) : undefined,
          close_time: Math.floor(Date.now() / 1000)
        }
        const res = await closePositionApi(pos.id, payload)
        if (res && res.code === 1) {
          const pnl = Number(res?.data?.realized_pnl || 0)
          const pct = Number(res?.data?.realized_pnl_percent || 0)
          this.$message.success(`平仓成功：${pnl >= 0 ? '+' : ''}${this.getCurrencySymbol(pos.market)}${this.formatNumber(pnl)} (${pct >= 0 ? '+' : ''}${pct.toFixed(2)}%)`)
          await this.loadData()
          if (this.showClosedPositionsModal) {
            await this.loadClosedPositions()
          }
        } else {
    ...
    async openClosedPositionsModal () {
      this.showClosedPositionsModal = true
      await this.loadClosedPositions()
    },
    async loadClosedPositions () {
      this.loadingClosedPositions = true
      try {
        const res = await getClosedPositions({ page: 1, page_size: 200 })
        if (res && res.code === 1) {
          this.closedPositions = res.data?.items || []
        } else {
        ...
        this.loadingClosedPositions = false
      }
    },
    closePositionModal () {
