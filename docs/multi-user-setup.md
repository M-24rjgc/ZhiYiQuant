...
This guide explains how to configure ZhiYiQuant for multi-user mode with PostgreSQL database.
...
cat > .env << EOF
POSTGRES_USER=zhiyiquant
POSTGRES_PASSWORD=your_secure_password_here
POSTGRES_DB=zhiyiquant
EOF
```
...
CREATE DATABASE zhiyiquant;
CREATE USER zhiyiquant WITH ENCRYPTED PASSWORD 'your_password';
GRANT ALL PRIVILEGES ON DATABASE zhiyiquant TO zhiyiquant;
\q
```
...
```bash
# Run init.sql
psql -U zhiyiquant -d zhiyiquant -f backend_api_python/migrations/init.sql
```
...
DB_TYPE=postgresql
DATABASE_URL=postgresql://zhiyiquant:your_password@localhost:5432/zhiyiquant
...
export DATABASE_URL=postgresql://zhiyiquant:your_password@localhost:5432/zhiyiquant
...
# Check connection
psql -U zhiyiquant -d zhiyiquant -c "SELECT 1"
```
...
```bash
# Check SQLite path
ls -la backend_api_python/data/zhiyiquant.db

# Check PostgreSQL tables
psql -U zhiyiquant -d zhiyiquant -c "\dt"
```
