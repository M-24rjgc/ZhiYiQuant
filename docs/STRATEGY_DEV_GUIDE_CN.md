# ZhiYiQuant Python 策略开发指南

本指南将详细介绍如何在 ZhiYiQuant 平台中使用 Python 开发交易策略。ZhiYiQuant 提供了灵活的执行环境，支持数据访问、指标计算和信号生成。

## 1. 概览

ZhiYiQuant 中的策略基于 **信号提供者 (Signal Provider)** 模式运行。系统执行你的 Python 脚本，该脚本负责处理市场数据（DataFrame）并输出交易信号。
...
**关于信号类型的说明:**
*   ZhiYiQuant 会根据你的策略配置（仅做多、仅做空或双向）来标准化信号。
