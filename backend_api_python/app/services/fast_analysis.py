...
class FastAnalysisService:
    ...
    def _normalize_symbol_for_market(self, market: str, symbol: str) -> Optional[str]:
        """Normalize and validate symbol format for specific markets."""
        m = (market or '').strip()
        ...
        if m == 'AShare':
            # A-share should be pure digits (typically 6-digit code)
    ...
    def _build_analysis_prompt(self, data: Dict[str, Any], language: str) -> tuple:
        ...
        system_prompt = f"""You are ZhiYiQuant's Senior Financial Analyst with 20+ years of experience. 
...
Provide your analysis now. Remember: all prices must be within 10% of ${current_price}."""
    ...
    def analyze(self, market: str, symbol: str, language: str = 'en-US', 
        ...
        try:
            normalized_symbol = self._normalize_symbol_for_market(market, symbol)
            if not normalized_symbol:
                if market == 'HShare':
                    result["error"] = "Invalid HShare symbol format. Please use HK numeric code (e.g., 00700 or 0700.HK)."
                elif market == 'AShare':
                    result["error"] = "Invalid AShare symbol format. Please use 6-digit numeric code (e.g., 600519)."
                else:
