...
def _enforce_single_client_login() -> bool:
    """
    Whether to invalidate old sessions on every new login.

    - true: single-client mode (new login kicks old token)
    - false: allow multiple concurrent sessions
    """
    return os.getenv('SINGLE_CLIENT_LOGIN', 'true').lower() == 'true'
...
def login():
    ...
    try:
        ...
        # Step 4: token_version strategy (single-client or multi-session)
        user_id = user.get('id') or user.get('user_id', 1)
        try:
            from app.services.user_service import get_user_service
            user_service = get_user_service()
            if _enforce_single_client_login():
                new_token_version = user_service.increment_token_version(user_id)
            else:
...
def login_with_code():
    ...
    try:
        ...
        # token_version strategy (single-client or multi-session)
        try:
            if _enforce_single_client_login():
