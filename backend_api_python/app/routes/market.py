...
def _normalize_symbol_for_market(market: str, symbol: str) -> str:
    """Normalize and validate symbol according to market conventions."""
    m = (market or '').strip()
    ...
    if m == 'HShare':
        # Accept: 700 / 0700 / 00700 / 0700.HK / HK0700
        if s.endswith('.HK'):
            s = s[:-3]
        if s.startswith('HK'):
            s = s[2:]
    ...
    if m == 'AShare':
        # Accept numeric stock code only
...
def get_menu_footer_config():
    ...
    data = {
        ...
        'copyright': '© 2025-2026 ZhiYiQuant'
    }
    return jsonify({'code': 1, 'msg': 'success', 'data': data})
...
def add_watchlist():
    ...
    try:
        ...
        symbol = _normalize_symbol_for_market(market, data.get('symbol'))
        name_in = (data.get('name') or '').strip()
        if not market or not symbol:
            if market == 'HShare':
                return jsonify({'code': 0, 'msg': 'Invalid HShare symbol. Please use numeric code, e.g., 00700 or 0700.HK', 'data': None}), 400
            if market == 'AShare':
                return jsonify({'code': 0, 'msg': 'Invalid AShare symbol. Please use 6-digit numeric code, e.g., 600519', 'data': None}), 400
...
@market_bp.route('/watchlist/batch-add', methods=['POST'])
@login_required
def batch_add_watchlist():
    """Batch add symbols to watchlist for current user."""
    try:
        user_id = g.user_id
        data = request.get_json() or {}
        ...
        if not isinstance(items, list) or len(items) == 0:
            return jsonify({'code': 0, 'msg': 'items must be a non-empty list', 'data': None}), 400
            ...
            for idx, item in enumerate(items):
                try:
                    if not isinstance(item, dict):
                        failed_items.append({'index': idx, 'reason': 'invalid_item'})
                        continue

                    market = (item.get('market') or '').strip()
                    symbol = _normalize_symbol_for_market(market, item.get('symbol'))
                    name_in = (item.get('name') or '').strip()

                    if not market or not symbol:
                        failed_items.append({'index': idx, 'reason': 'invalid_market_or_symbol'})
                        continue
                    ...
                    failed_items.append({'index': idx, 'reason': str(ie)})
        ...
        logger.error(f"batch_add_watchlist failed: {str(e)}")
        logger.error(traceback.format_exc())
        return jsonify({'code': 0, 'msg': str(e), 'data': None}), 500
...
def get_single_price(market: str, symbol: str) -> dict:
    """获取单个标的的价格数据"""
    try:
        normalized_symbol = _normalize_symbol_for_market(market, symbol)
