...
def _to_bool(v: Any) -> bool:
    ...
    if isinstance(v, (int, float)):
        return float(v) != 0.0
    s = str(v).strip().lower()
    return s in ('1', 'true', 'yes', 'y', 'on', 'buy', 'long', 'open')


def _kline_to_df(klines: List[Any]) -> pd.DataFrame:
    rows = []
    for item in (klines or []):
        if isinstance(item, dict):
            rows.append({
                'time': int(item.get('time') or item.get('timestamp') or 0),
...
def _run_indicator_code(code: str, df: pd.DataFrame, params: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {'ok': False, 'error': f'Runtime Error: {str(e)}'}

    output = exec_env.get('output')
    if output is None and 'result_json' in exec_env:
        try:
            output = json.loads(exec_env.get('result_json') or '{}')
        except Exception:
            output = None
    ...
    if isinstance(output, str):
        try:
            ...
            return {'ok': False, 'error': 'output is string but not valid JSON'}

    if not isinstance(output, dict):
        return {'ok': False, 'error': "Missing or invalid 'output' dictionary"}

    return {'ok': True, 'output': output}


def _score_entry(df: pd.DataFrame, output: Dict[str, Any], min_score: int) -> Dict[str, Any]:
    ...
    for s in signals:
        stype = str((s or {}).get('type') or '').strip().lower()
        data = (s or {}).get('data') or []
        if not isinstance(data, list) or len(data) <= idx:
            continue

        now_v = _to_bool(data[idx])
        prev_v = _to_bool(data[prev_idx]) if len(data) > prev_idx else False

        is_buy_type = any(k in stype for k in ['buy', 'long', 'open', 'entry', 'bull'])
        is_sell_type = any(k in stype for k in ['sell', 'short', 'close', 'exit', 'bear'])

        if is_buy_type and now_v:
            buy_now = True
            if not prev_v:
                buy_new = True
        if is_sell_type and now_v:
            sell_now = True
    ...
    if buy_now:
        score += 45
        reasons.append('当前K线存在买入信号')
    if buy_new:
        score += 25
        reasons.append('买入信号刚触发（边沿触发）')
    if sell_now:
        score -= 60
        reasons.append('当前K线存在卖出信号，抵消入场')

    if len(closes) >= 20:
        ma20 = float(pd.Series(closes).rolling(20).mean().iloc[-1] or 0)
        if ma20 > 0 and last_close > ma20:
            score += 15
            reasons.append('价格位于MA20上方')

    if len(closes) >= 6:
        momentum = last_close - float(closes[-6])
        if momentum > 0:
            score += 10
            reasons.append('短期动能为正')

    if abs(change_pct) > 7:
        score -= 5
        reasons.append('短线波动较大，适当降分')
    ...
    is_entry = bool(buy_now and not sell_now and score >= int(min_score))
    return {
        'is_entry': is_entry,
        'score': int(score),
        'grade': grade,
...
@indicator_bp.route("/scanOnce", methods=["POST"])
@login_required
def scan_once():
    """
    单次批量扫描：使用指定指标，对股票池进行一次性扫描并返回当前入场机会。
    不创建持续任务。
    """
    try:
        data = request.get_json() or {}
        ...
        indicator_id = int(data.get('indicator_id') or 0)
        if indicator_id <= 0:
            return jsonify({'code': 0, 'msg': 'indicator_id is required', 'data': None}), 400

        universe_type = str(data.get('universe_type') or 'watchlist').strip().lower()
        market = str(data.get('market') or 'USStock').strip()
        timeframe = str(data.get('timeframe') or '1D').strip()
        kline_limit = max(80, min(600, int(data.get('kline_limit') or 220)))
        max_symbols = max(1, min(1000, int(data.get('max_symbols') or 60)))
        min_score = max(0, min(100, int(data.get('min_score') or 55)))
        ...
        watchlist_set = set((str(r.get('market') or ''), str(r.get('symbol') or '').upper()) for r in watchlist_rows)

        symbol_items: List[Dict[str, Any]] = []
        if isinstance(data.get('symbols'), list) and data.get('symbols'):
            for s in data.get('symbols'):
                if isinstance(s, dict):
                    mk = str(s.get('market') or market).strip()
                    ...
                    nm = ''
                if mk and sym:
                    symbol_items.append({'market': mk, 'symbol': sym, 'name': nm})
        elif universe_type in ('market_all', 'all', 'seed_all'):
            try:
                with get_db_connection() as db:
                    cur = db.cursor()
                ...
                # 回退：至少给出热门池，避免全量表不可用时直接失败
                fallback_market = market if (market and market.upper() not in ('ALL', '*')) else 'USStock'
                hot = seed_get_hot_symbols(market=fallback_market, limit=max_symbols) or []
                for h in hot:
                    mk = str(h.get('market') or fallback_market).strip()
                    sym = str(h.get('symbol') or '').strip().upper()
                    nm = str(h.get('name') or '').strip()
                    if mk and sym:
                        symbol_items.append({'market': mk, 'symbol': sym, 'name': nm})
        elif universe_type == 'hot':
            hot = seed_get_hot_symbols(market=market, limit=max_symbols) or []
            for h in hot:
                mk = str(h.get('market') or market).strip()
                sym = str(h.get('symbol') or '').strip().upper()
                nm = str(h.get('name') or '').strip()
                if mk and sym:
                    symbol_items.append({'market': mk, 'symbol': sym, 'name': nm})
        ...
        if not uniq:
            return jsonify({'code': 0, 'msg': 'No symbols to scan', 'data': {'hits': [], 'errors': []}}), 400

        code = str(indicator.get('code') or '')
        if not code.strip():
            return jsonify({'code': 0, 'msg': 'Indicator code is empty', 'data': None}), 400
        ...
        def _scan_one(item: Dict[str, Any]) -> Dict[str, Any]:
            mk = item.get('market')
            sym = item.get('symbol')
            nm = item.get('name') or sym
            try:
                klines = kline_service.get_kline(mk, sym, timeframe, kline_limit)
                df = _kline_to_df(klines)
                if len(df) < 50:
                    return {'ok': False, 'market': mk, 'symbol': sym, 'name': nm, 'error': 'insufficient_kline'}

                run_ret = _run_indicator_code(code, df, {})
                if not run_ret.get('ok'):
                    return {'ok': False, 'market': mk, 'symbol': sym, 'name': nm, 'error': run_ret.get('error')}

                scored = _score_entry(df, run_ret.get('output') or {}, min_score)
                if not scored.get('is_entry'):
                    return {'ok': False, 'market': mk, 'symbol': sym, 'name': nm, 'error': 'no_entry_signal', 'score': scored.get('score', 0)}

                key = (str(mk or ''), str(sym or '').upper())
                return {
                    'ok': True,
                    'item': {
                        'market': mk,
                        'symbol': sym,
                        'name': nm,
                        'latest_price': scored.get('last_close', 0),
                        'change_pct': scored.get('change_pct', 0),
                        'score': scored.get('score', 0),
                        'grade': scored.get('grade', 'C'),
                        'reasons': scored.get('reasons', []),
                        'buy_now': scored.get('buy_now', False),
                        'buy_new': scored.get('buy_new', False),
                        'sell_now': scored.get('sell_now', False),
                        'in_watchlist': key in watchlist_set
                    }
                }
            except Exception as ex:
                return {'ok': False, 'market': mk, 'symbol': sym, 'name': nm, 'error': str(ex)}

        worker_count = max(4, min(12, (len(uniq) // 40) + 4))
        with ThreadPoolExecutor(max_workers=worker_count) as pool:
            futures = [pool.submit(_scan_one, it) for it in uniq]
            for f in as_completed(futures):
                ret = f.result()
                if ret.get('ok'):
                    hits.append(ret.get('item'))
                else:
                    errors.append({
                        'market': ret.get('market'),
                        'symbol': ret.get('symbol'),
                        'name': ret.get('name') or ret.get('symbol'),
                        'error': ret.get('error') or 'scan_failed',
                        'score': ret.get('score', 0)
                    })

        hits.sort(key=lambda x: (float(x.get('score') or 0), abs(float(x.get('change_pct') or 0))), reverse=True)
        ...
        logger.error(f"scan_once failed: {str(e)}", exc_info=True)
        return jsonify({'code': 0, 'msg': str(e), 'data': None}), 500
