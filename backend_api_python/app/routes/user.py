...
def test_notification():
    """
    Send a real test notification to the user's configured channels.
    Reads SMTP config from os.environ (synced by save_settings),
    reads user's notification_settings from DB.
    """
    try:
        ...
        # Get user's notification settings from DB
        with get_db_connection() as db:
            cur = db.cursor()
            cur.execute(
                "SELECT notification_settings, email, username FROM qd_users WHERE id = ?",
                (user_id,)
            )
            row = cur.fetchone()
            cur.close()
        ...
        ns_raw = row.get('notification_settings') or row.get(0) or '{}'
        user_email = row.get('email') or row.get(1) or ''
        username = row.get('username') or row.get(2) or 'User'

        try:
            ns = json.loads(ns_raw) if isinstance(ns_raw, str) else (ns_raw or {})
        except Exception:
            ns = {}

        channels = ns.get('default_channels', ['browser'])

        # Create a fresh notifier (re-reads os.environ)
        ...
        if 'email' in channels:
            target_email = ns.get('email') or user_email
            if not target_email:
                results['email'] = {'ok': False, 'error': '未配置通知邮箱'}
            else:
                ok, err = notifier._notify_email(
                    to_email=target_email,
                    subject='ZhiYiQuant 测试通知',
                    body_text=f'Hi {username}, 这是一封来自 ZhiYiQuant 的测试邮件。如果您收到此邮件，说明邮件通知配置成功！',
                    body_html=f'<h3>ZhiYiQuant 测试通知</h3>'
                              f'<p>Hi <b>{username}</b>,</p>'
                              f'<p>这是一封来自 ZhiYiQuant 的测试邮件。</p>'
                              f'<p>如果您收到此邮件，说明邮件通知配置成功！ ✅</p>'
                )
                results['email'] = {'ok': ok, 'error': err if not ok else ''}
                if ok:
                    any_success = True
        ...
        if 'telegram' in channels:
            ...
            bot_token = ns.get('telegram_bot_token', '')
            if not chat_id:
                results['telegram'] = {'ok': False, 'error': '未配置 Telegram Chat ID'}
            else:
                ok, err = notifier._notify_telegram(
                    chat_id=chat_id,
                    text=f'✅ ZhiYiQuant 测试通知\n\nHi {username}, Telegram 通知已成功配置！',
                    token_override=bot_token or ''
                )
                results['telegram'] = {'ok': ok, 'error': err if not ok else ''}
                if ok:
                    any_success = True

        # Test discord
        if 'discord' in channels:
            webhook_url = ns.get('discord_webhook', '')
            if not webhook_url:
                results['discord'] = {'ok': False, 'error': '未配置 Discord Webhook URL'}
            else:
                ok, err = notifier._notify_discord(
                    url=webhook_url,
                    payload={},
                    fallback_text=f'✅ ZhiYiQuant 测试通知 - Hi {username}, Discord 通知已成功配置！'
                )
                results['discord'] = {'ok': ok, 'error': err if not ok else ''}
                if ok:
                    any_success = True

        if not any(ch in channels for ch in ['email', 'telegram', 'discord']):
            return jsonify({
                'code': 0,
                'msg': '请至少选择 email / telegram / discord 其中一个可发送的通知渠道',
                'data': {'results': results}
            })

        return jsonify({
            'code': 1 if any_success else 0,
            'msg': '测试通知已发送' if any_success else '测试通知发送失败',
            'data': {'results': results}
        })
    except Exception as e:
        logger.error(f"test_notification failed: {e}")
        return jsonify({'code': 0, 'msg': f'发送失败: {str(e)}', 'data': None}), 500
