...
<script>
import { getProfile, updateProfile, getMyCreditsLog, getMyReferrals, getNotificationSettings, updateNotificationSettings, testNotification } from '@/api/user'
  ...
  methods: {
    ...
    async handleTestNotification () {
      ...
      try {
        ...
        // Call backend to actually send test notification
        const testRes = await testNotification()
        if (testRes.code === 1) {
          const results = (testRes.data && testRes.data.results) || {}
          const details = Object.entries(results)
            .map(([ch, r]) => `${ch}: ${r.ok ? '✅ 成功' : '❌ ' + (r.error || '失败')}`)
            .join('；')
          this.$message.success(details || '测试通知已发送，请检查您的通知渠道')
        } else {
          const results = (testRes.data && testRes.data.results) || {}
          const details = Object.entries(results)
            .map(([ch, r]) => `${ch}: ${r.error || '失败'}`)
            .join('；')
          this.$message.error(details || testRes.msg || '测试通知发送失败')
        }
      } catch (e) {
