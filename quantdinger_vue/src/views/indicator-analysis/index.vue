<template>
  <div class="chart-container" :class="{ 'theme-dark': isDarkTheme }">
    ...
    <div class="chart-content">
          ...
          <div class="indicators-panel">
            <div class="panel-header">
              ...
              <div style="display: flex; align-items: center; margin-left: auto; gap: 8px;">
                <a-button size="small" @click="openScanModal">
      ...
      >
        <div>
          <a-form layout="inline" style="margin-bottom: 12px; row-gap: 8px;">
            <a-form-item label="指标">
              <a-select v-model="scanForm.indicator_id" style="width: 260px" placeholder="请选择指标">
                <a-select-option v-for="opt in scanIndicatorOptions" :key="opt.value" :value="opt.value">
                  {{ opt.label }}
                </a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item label="范围">
              <a-select v-model="scanForm.universe_type" style="width: 120px">
                <a-select-option value="watchlist">自选股</a-select-option>
                <a-select-option value="hot">热门池</a-select-option>
                <a-select-option value="market_all">全市场</a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item v-if="scanForm.universe_type === 'hot' || scanForm.universe_type === 'market_all'" label="市场">
              <a-select v-model="scanForm.market" style="width: 120px">
                <a-select-option v-if="scanForm.universe_type === 'market_all'" value="ALL">全部市场</a-select-option>
                <a-select-option v-for="mt in marketTypes" :key="mt.value" :value="mt.value">
                  {{ $t(mt.i18nKey || `dashboard.analysis.market.${mt.value}`) }}
                </a-select-option>
              </a-select>
            </a-form-item>
            <a-form-item label="周期">
              <a-select v-model="scanForm.timeframe" style="width: 90px">
            ...
            <a-form-item label="最小分数">
              <a-input-number v-model="scanForm.min_score" :min="0" :max="100" :step="1" style="width: 90px" />
            </a-form-item>
            <a-form-item label="数量上限">
              <a-input-number v-model="scanForm.max_symbols" :min="5" :max="1000" :step="20" style="width: 100px" />
            </a-form-item>
            <a-form-item>
              <a-button type="primary" :loading="scanningOnce" @click="runScanOnce">
                <a-icon type="play-circle" /> 开始扫描
              </a-button>
            </a-form-item>
          </a-form>

          <div v-if="scanSummary" style="margin-bottom: 12px; color: #666;">
            已扫描 {{ scanSummary.total_symbols || 0 }} 个，命中 {{ scanSummary.matched_count || 0 }} 个
          </div>
          ...
          <div style="margin-bottom: 12px;">
            <a-button
              ...
              :loading="addingScanToWatchlist"
              @click="addScanHitsToWatchlist"
            >
              <a-icon type="star" /> 一键加入我的自选股（{{ scanResults.length }}）
            </a-button>
          </div>
          ...
          <a-table
            ...
            size="small"
          >
            <a-table-column title="标的" key="symbol">
              <template slot-scope="text, record">
                <a-tag :color="getMarketColor(record.market)">{{ getMarketName(record.market) }}</a-tag>
                <span>{{ record.symbol }}</span>
                <span v-if="record.name" style="color:#999;margin-left:6px;">{{ record.name }}</span>
              </template>
            </a-table-column>
            <a-table-column title="最新价" key="latest_price">
              <template slot-scope="text, record">
                {{ Number(record.latest_price || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 6 }) }}
              </template>
            </a-table-column>
            <a-table-column title="分数" key="score">
              <template slot-scope="text, record">
                <a-tag :color="record.score >= 70 ? 'green' : (record.score >= 55 ? 'blue' : 'orange')">
                  {{ record.score }} ({{ record.grade }})
                </a-tag>
              </template>
            </a-table-column>
            <a-table-column title="涨跌%" key="change_pct">
              <template slot-scope="text, record">
                <span :style="{ color: Number(record.change_pct || 0) >= 0 ? '#16a34a' : '#dc2626' }">
                  {{ Number(record.change_pct || 0) >= 0 ? '+' : '' }}{{ Number(record.change_pct || 0).toFixed(2) }}%
                </span>
              </template>
            </a-table-column>
            <a-table-column title="理由" key="reasons">
              <template slot-scope="text, record">
          ...
          <a-empty v-if="!scanningOnce && scanResults.length === 0" description="暂无命中结果" style="margin-top: 12px;" />
        </div>
      </a-modal>
...
import { getWatchlist, addWatchlist, addWatchlistBatch, searchSymbols, getHotSymbols, getMarketTypes } from '@/api/market'
...
export default {
  ...
  setup () {
    ...
    const scanningOnce = ref(false)
    const addingScanToWatchlist = ref(false)
    const scanForm = ref({
      indicator_id: undefined,
      universe_type: 'watchlist', // watchlist | hot | market_all
      market: 'USStock',
      timeframe: '1D',
      min_score: 55,
      max_symbols: 60,
    ...
    const scanIndicatorOptions = computed(() => {
      const custom = (customIndicators.value || []).map(i => ({
        value: i.id,
        label: `[我创建] ${i.name}`
      }))
      const purchased = (purchasedIndicators.value || []).map(i => ({
        value: i.id,
        label: `[我购买] ${i.name}`
      }))
      return [...custom, ...purchased]
    })
    ...
    const loadIndicators = async () => {
      ...
      try {
        ...
        if (res.code === 1 && res.data) {
          ...
          if (!scanForm.value.indicator_id) {
            const first = (customItems[0] || purchasedItems[0])
            if (first && first.id) {
              scanForm.value.indicator_id = first.id
            }
          }
        }
      } catch (error) {
    ...
    const openScanModal = () => {
      if (scanIndicatorOptions.value.length === 0) {
        message.warning('请先创建或购买至少一个指标')
        return
      }
      if (!scanForm.value.indicator_id) {
        scanForm.value.indicator_id = scanIndicatorOptions.value[0]?.value
      }
    ...
    const runScanOnce = async () => {
      ...
      scanningOnce.value = true
      scanResults.value = []
      scanSummary.value = null
      try {
        const res = await request({
          url: '/api/indicator/scanOnce',
          method: 'post',
          data: {
            indicator_id: scanForm.value.indicator_id,
            universe_type: scanForm.value.universe_type,
            market: scanForm.value.market,
            timeframe: scanForm.value.timeframe,
            min_score: scanForm.value.min_score,
            max_symbols: scanForm.value.max_symbols,
            kline_limit: scanForm.value.kline_limit
          }
        })

        if (res && res.code === 1) {
          scanResults.value = res.data?.hits || []
          scanSummary.value = {
            total_symbols: res.data?.total_symbols || 0,
            matched_count: res.data?.matched_count || 0
          }
          if ((res.data?.matched_count || 0) > 0) {
            message.success(`扫描完成，命中 ${res.data.matched_count} 个机会`)
          } else {
            message.info('扫描完成，当前无高分入场机会')
          }
        } else {
        ...
        scanningOnce.value = false
      }
    }

    const addScanHitsToWatchlist = async () => {
      if (!scanResults.value || scanResults.value.length === 0) {
        message.warning('没有可加入自选股的扫描结果')
        return
      }
      addingScanToWatchlist.value = true
      try {
        const items = scanResults.value.map(i => ({
          market: i.market,
          symbol: i.symbol,
          name: i.name || ''
        }))
        const res = await addWatchlistBatch({ items })
        if (res && res.code === 1) {
          const added = Number(res.data?.added || 0)
          const updated = Number(res.data?.updated || 0)
          message.success(`已加入自选股：新增 ${added}，已存在更新 ${updated}`)
          await loadWatchlist()
          // 更新扫描结果中的 in_watchlist 标记
          scanResults.value = scanResults.value.map(i => ({ ...i, in_watchlist: true }))
        } else {
          message.error(res?.msg || '加入自选股失败')
        }
      } catch (e) {
        message.error('加入自选股失败')
      } finally {
        addingScanToWatchlist.value = false
      }
    }
    ...
    // 监听扫描范围切换，给出更合理默认值
    watch(
      () => scanForm.value.universe_type,
      (val) => {
        if (val === 'market_all') {
          scanForm.value.market = 'ALL'
          if (Number(scanForm.value.max_symbols || 0) < 200) {
            scanForm.value.max_symbols = 200
          }
        } else if (scanForm.value.market === 'ALL') {
          scanForm.value.market = 'USStock'
        }
      }
    )
...
loadingWatchlist,
      ...
      showScanModal,
      scanningOnce,
      addingScanToWatchlist,
      scanForm,
      scanResults,
      scanSummary,
      scanIndicatorOptions,
      realtimeEnabled,
toggleRealtime,
      handleSymbolSearch,
      handleSymbolSelect,
