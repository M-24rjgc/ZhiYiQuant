...
function resolveBaseURL () {
  if (typeof window !== 'undefined') {
    const injected = window.__QD_API_BASE_URL__
    if (typeof injected === 'string' && injected.trim()) {
      return injected.trim()
    }
    const isTauri = !!(window.__TAURI__ || window.__TAURI_IPC__ || window.__TAURI_INTERNALS__)
    if (isTauri) {
      const port = window.__QD_API_PORT__ || 5000
  ...
  if (phpsessid && typeof document !== 'undefined') {
    ...
    if (!currentPhpsessid || currentPhpsessid !== phpsessid) {
      ...
      try {
        ...
        if (window.location.hostname.includes('zhiyiquant.com')) {
          document.cookie = `PHPSESSID=${phpsessid}; path=/; domain=.zhiyiquant.com; SameSite=None; Secure`
        } else {
